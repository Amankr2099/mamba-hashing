...
# ... [Keep your existing imports and path setup for VIM_SRC_DIR] ...

# -------------------------------------------------------------------------
# HELPER FUNCTION: RESIZE POSITION EMBEDDINGS
# -------------------------------------------------------------------------
def resize_pos_embed(posemb, posemb_new, num_tokens=1, gs_new=None):
    """
    Rescale the grid of position embeddings when loading from a different resolution.
    Args:
        posemb: The pos_embed from the checkpoint (1, seq_len_old, dim)
        posemb_new: The pos_embed from the current model (1, seq_len_new, dim)
        num_tokens: Number of special tokens (CLS, etc.) at the beginning.
    """
    # If shapes match, return immediately
    if posemb.shape == posemb_new.shape:
        return posemb

    print(f"Resizing pos_embed from {posemb.shape} to {posemb_new.shape}")
    
    ntok_new = posemb_new.shape[1]
    
    # Separate the special tokens (like CLS) from the grid tokens
    if num_tokens:
        posemb_tok, posemb_grid = posemb[:, :num_tokens], posemb[:, num_tokens:]
        ntok_new -= num_tokens
    else:
        posemb_tok, posemb_grid = None, posemb
    ...
    print(f"Grid resize: {gs_old}x{gs_old} -> {gs_new}x{gs_new}")

    # Reshape grid to (1, Dim, H, W) for F.interpolate
    posemb_grid = posemb_grid.reshape(1, gs_old, gs_old, -1).permute(0, 3, 1, 2)
    
    # Perform bicubic interpolation
    posemb_grid = F.interpolate(posemb_grid, size=(gs_new, gs_new), mode='bicubic', align_corners=False)
    
    # Reshape back to (1, Seq_Len, Dim)
...
class VisionMambaHashing(nn.Module):
    # ... [Keep your __init__ and forward methods exactly as they are] ...
    ...
    def load_from(self, weights_path):
        ...
        if 'pos_embed' in state_dict and hasattr(self.model, 'pos_embed'):
            # Check for size mismatch
            if state_dict['pos_embed'].shape != self.model.pos_embed.shape:
                # Assuming 1 CLS token based on 'if_cls_token=True' in config
                # The error log showed mismatch: Checkpoint [1, 730, 192] vs Model [1, 197, 192]
                try:
                    state_dict['pos_embed'] = resize_pos_embed(
                        state_dict['pos_embed'], 
                        self.model.pos_embed, 
                        num_tokens=1 # Usually 1 for ViT/Vim CLS token
                    )
                except Exception as e:
                    print(f"Failed to resize pos_embed: {e}. Skipping pos_embed loading.")
